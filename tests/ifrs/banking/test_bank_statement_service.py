...
class TestGetUnmatchedLines:
    ...
    def test_get_unmatched_lines(self, service, mock_db, org_id):
        ...
        result = service.get_unmatched_lines(mock_db, org_id, statement.statement_id)
...
class TestDeleteStatement:
    ...
    def test_delete_statement_success(self, service, mock_db, org_id):
        """Test deleting a statement."""
        statement = MockBankStatement(status="imported", organization_id=org_id)
        mock_db.get.return_value = statement

        result = service.delete(mock_db, org_id, statement.statement_id)

        assert result is True
        mock_db.delete.assert_called_once_with(statement)

    def test_delete_nonexistent_statement(self, service, mock_db, org_id):
        """Test deleting non-existent statement returns False."""
        mock_db.get.return_value = None

        result = service.delete(mock_db, org_id, uuid4())

        assert result is False

    def test_delete_reconciled_statement_fails(self, service, mock_db, org_id):
        """Test deleting reconciled statement fails."""
        from app.models.finance.banking.bank_statement import BankStatementStatus

        statement = MockBankStatement(
            status=BankStatementStatus.reconciled, organization_id=org_id
        )
        mock_db.get.return_value = statement

        with pytest.raises(ValueError, match="Cannot delete"):
            service.delete(mock_db, org_id, statement.statement_id)
